...
//=============================================================================
 /*:
 * @plugindesc v1.30 物品核心修改版，可改变物品场景并加强处理。
...
Game_Actor.prototype.changeEquipById = function(etypeId, itemId) {
    if (itemId > 0) {
        ...
        if (!$gameParty.hasItem(baseItem)) $gameParty.gainItem(baseItem, 1);
...
Window_ItemList.prototype.ItemColor = function(item) {
...
Window_ItemInfo.prototype.drawItemName = function(item, x, y, width) {
    width = width || 312;
    if (item) {
        var iconBoxWidth = Window_Base._iconWidth + 4;
        this.ItemColor(item);
...
Window_ItemInfo.prototype.ItemColor = function(item) {
    if (DataManager.isItem(item)) return;
    var quality = item._quality;
    if (quality === 'Unique') this.changeTextColor(this.textColor(14));
    else if (quality === 'Rare') this.changeTextColor(this.textColor(30));
