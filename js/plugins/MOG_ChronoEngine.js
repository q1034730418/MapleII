...
ToolEvent.prototype.getInitialPosition = function (target) {
    if (!target) {
        return [0, 0]
    } else {
        var x = target._realX || target.x || 0
        var y = target._realY || target.y || 0
        var h = target.jumpHeight ? (target.jumpHeight() / $gameMap.tileHeight()) : 0;
        return [x, y - h]
    }
}
...
ToolEvent.prototype.setInitialPosition = function () {
    ...
    if (this.user()._user.autoTarget && this._tool.position === 1) {
        var xy = this.getInitialPosition(this.user()._user.autoTarget)
        var x = xy[0];
        var y = xy[1];
    } else if (this.user()._user.chainActionHit[4]) {
        var xy = this.getInitialPosition(this.user()._user.chainActionHit[4])
        var x = xy[0];
        var y = xy[1];
    } else {
        var xy = this.getInitialPosition(this.user())
        var x = xy[0];
        var y = xy[1];
        ;
    };
...
ToolEvent.prototype.inRange = function (dx, dy) {
    ...
    if (mode === 0) {
        ...
        if (this._direction === 2) {
            ...
            if (Math.abs(dx) + Math.abs(dy) <= range && dy <= 0 && dx >= 0) { return true };
        } else if (this._direction === 4) {
            if (Math.abs(dx) + Math.abs(dy) <= range && dy <= 0 && dx <= 0) { return true };
        }; 
        // 半口
