...
Scene_MenuBase.prototype.terminate = function() {
	_drill_MGi_terminate.call(this);			//设置需要下次重新创建
	SceneManager._drill_MGi_created = false;
};
//==============================
// ** 菜单 - 层级排序
...
Scene_MenuBase.prototype.update = function() {
	...
	if( SceneManager.isCurrentSceneStarted() && !SceneManager._drill_MGi_created ) {
		this.drill_MGi_create();				//创建，进入界面后只执行一次
	}
	if( SceneManager._drill_MGi_created ){
...
Scene_MenuBase.prototype.drill_MGi_create = function() {	
	...
	for (var i = 0; i < DrillUp.g_MGi_list.length; i++) {
		if( this.drill_MGi_checkKeyword(i) ){
			// > GIF贴图
			var temp_sprite_data = JSON.parse(JSON.stringify( DrillUp.g_MGi_list[i] ));	//拷贝object（杜绝引用造成的修改）
