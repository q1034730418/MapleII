 ...
 * 使游戏得以模仿 ARPG 效果，如配合作者修改的 MOG_LMBS.js 插件使用，
 ...
 *
 * 如遇有些插件生成菜单不显示方向盘与按钮，或者点击无响应，可作如下调整：
...
for (var i = 0; i < 20; i++) {
    ...
    MobileJoystick.mapBtnActive[i] = String(MobileJoystick.mapBtn[i]['启用按钮']).toLowerCase() == 'true';
    ...
    MobileJoystick.mapBtnMode[i] = String(MobileJoystick.mapBtn[i]['触发模式']).toLowerCase() == 'true';
...
for (var i = 0; i < 15; i++) {
    ...
    MobileJoystick.btlBtnActive[i] = String(MobileJoystick.btlBtn[i]['启用按钮']).toLowerCase() == 'true';
    ...
    MobileJoystick.btlBtnMode[i] = String(MobileJoystick.btlBtn[i]['触发模式']).toLowerCase() == 'true';
...
for (var i = 0; i < 5; i++) {
    ...
    MobileJoystick.menuBtnActive[i] = String(MobileJoystick.menuBtn[i]['启用按钮']).toLowerCase() == 'true';
    ...
    MobileJoystick.menuBtnMode[i] = String(MobileJoystick.menuBtn[i]['触发模式']).toLowerCase() == 'true';
...
Game_Temp.prototype.setDestination = function(x, y) {
    if (TouchInput.onButton() && MobileJoystick.Show) return;
    if (Utils.isMobileDevice() && !MobileJoystick.TouchMove && MobileJoystick.Show) return;
    if (MobileJoystick.PCJoystick && !MobileJoystick.TouchMove && MobileJoystick.Show) return;
    MobileJoystick.Game_Temp_setDestination.call(this, x, y);
};

MobileJoystick.Game_Interpreter_pluginCommand = Game_Interpreter.prototype.pluginCommand;
Game_Interpreter.prototype.pluginCommand = function(command, args){
    MobileJoystick.Game_Interpreter_pluginCommand.call(this, command, args);
    if (command === 'MBJopen') MobileJoystick.Show = true;
    if (command === 'MBJclose') MobileJoystick.Show = false;
};
...
Scene_Base.prototype.respondButton = function(index) {
    if (SceneManager._scene instanceof Scene_Map) {
        if (MobileJoystick.mapBtnType[index] == 0) $gameTemp.reserveCommonEvent(MobileJoystick.mapBtnEvent[index]);
            ...
            setTimeout(function(){Input._currentState[MobileJoystick.mapBtnKey[index]] = false;}, 20);
...
Scene_Base.prototype.updateOpacity = function() {
    if (MobileJoystick.Show) {
        if (this._dirPad) this._dirPad.opacity = 255;
        if (this._touchShadow) this._touchShadow.opacity = 255;
        for (var i = 0; i < 20; i++) {
            if (this._mapBtn && this._mapBtn[i]) this._mapBtn[i].opacity = 255;
            if (this._btlBtn && this._btlBtn[i]) this._btlBtn[i].opacity = 255;
            if (this._menuBtn && this._menuBtn[i]) this._menuBtn[i].opacity = 255;
        }
    } else {
        if (this._dirPad) this._dirPad.opacity = 0;
        if (this._touchShadow) this._touchShadow.opacity = 0;
        for (var i = 0; i < 20; i++) {
            if (this._mapBtn && this._mapBtn[i]) this._mapBtn[i].opacity = 0;
            if (this._btlBtn && this._btlBtn[i]) this._btlBtn[i].opacity = 0;
            if (this._menuBtn && this._menuBtn[i]) this._menuBtn[i].opacity = 0;
        }
    }
    this.MobileJoystickShow = MobileJoystick.Show;
};
...
Scene_Map.prototype.update = function() {
    ...
    if (this.MobileJoystickShow != MobileJoystick.Show) this.updateOpacity();
};
...
Window_Message.prototype.terminateMessage = function() {
    ...
    setTimeout("MobileJoystick.Message = false;", 200);//200
};
Window_TalkList.prototype.windowHeight = function () {
    switch (JHMW.OnlineCore.TalkSize) {
