...
Spriteset_Map.prototype.drill_ET_create = function() {
	...
	this._drill_ET_eventCount = 0;			//事件记录点
...
Spriteset_Map.prototype.drill_ET_updateNewEventText = function() {
	var s_tank = this._drill_ET_spriteTank;
	var e_tank = $gameTemp._drill_ET_textEvents;
	if( this._drill_ET_eventCount == e_tank.length ){ return; }		//只增不减
	...
	for(var i = 0; i < e_tank.length; i++ ){		//添加漂浮文字
		var e = e_tank[i];
		var hasBinded = false;
		for(var j = 0; j < s_tank.length; j++ ){	//排除已绑定的事件
			if( s_tank[j]._event == e ){
				hasBinded = true;
				break ;
			}
		}
		if( hasBinded == false ){
			this.drill_ET_createWindowSprite( e );
		}
	}
}
//==============================
...
Drill_ET_WindowSprite.prototype.standardPadding = function() {
    return DrillUp.g_ET_padding;
};
...
Drill_ET_WindowSprite.prototype.drill_ET_updateText = function() {
	...
	if( data_ET['_forceRefresh'] == true || this._drill_curText != String(data_ET['_text']) ){
		...
		this.width = this._drill_width + 8;		//稍微多几像素的空间
		...
		this.drawTextEx(this._drill_curText,4,-2);
