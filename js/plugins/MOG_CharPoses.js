...
Moghunter.charPoses_MFrames = String(
    Moghunter.parameters["Multi frames"] || "true"
);
Moghunter.charPoses_Player = String(
    Moghunter.parameters["Poses for Player"] || "true"
);
Moghunter.charPoses_Followers = String(
    Moghunter.parameters["Poses for Followers"] || "true"
);
Moghunter.charPoses_DashPose = String(
    Moghunter.parameters["Dash Pose"] || "true"
);
Moghunter.charPoses_JumpPose = String(
    Moghunter.parameters["Jump Pose"] || "true"
);
Moghunter.charPoses_IdlePose = String(
    Moghunter.parameters["Idle Pose"] || "true"
);
Moghunter.charPoses_IdlePoseTime = Number(
    Moghunter.parameters["Idle Start Time"] || 60
);
...
Game_System.prototype.initialize = function () {
    ...
    this._chaMPoses =
        String(Moghunter.charPoses_MFrames) === "true" ? true : false;
};
...
Window_Base.prototype.drawCharacter = function (
    ...
    if (frames) {
        this.drawCharacterPoses(characterName, characterIndex, x, y, frames[2]);
        return;
    }
    _mog_charPoses_drawCharacter.call(
        this,
        characterName,
        characterIndex,
        x,
...
//==============================
Window_Base.prototype.drawCharacterPoses = function (
    characterName,
    characterIndex,
    x,
    y,
...
Game_Character.prototype.refreshPoses = function () {
    ...
    if (this.battler()) {
        ...
        if (this._followers) {
            this._poses.enabled =
                String(Moghunter.charPoses_Player) === "true" ? true : false;
        } else if (this._memberIndex != null) {
            this._poses.enabled =
                String(Moghunter.charPoses_Followers) === "true" ? true : false;
        }
    }
};
...
Game_CharacterBase.prototype.isDashingPose = function () {
    ...
    if (this._memberIndex) {
        return this.isDashingPoseFollower();
    }
    if (!this.isMoving()) {
        return false;
    }
...
Game_CharacterBase.prototype.isDashingPoseFollower = function () {
    if (this._tool && $gameSystem.isChronoMode()) {
        return false;
    }
...
Game_CharacterBase.prototype.setJumpPose = function () {
    ...
    if (this.isDiagonalJumpPose()) {
        ...
        return this.getMorePose("jump");
        //$gameScreen.erasePicture(50);//图片
...
Game_CharacterBase.prototype.isIdlePose = function () {
    ...
    if (Imported.MOG_PickupThrow && $gamePlayer._pickup.enabled) {
        return false;
    }
    return true;
};
...
Game_CharacterBase.prototype.setKnockbackPose = function () {
    if (this.battlerPoses()) {
        this._poses.idle[1] = this._poses.idle[2];
        return this.getMorePose("die(f1)");
        //$gameScreen.erasePicture(50);//图片
        return this._originalName.name + "_die(f1)";
    }
    return "";
};
...
Game_CharacterBase.prototype.setActionPose = function () {
    if (this.battlerPoses()) {
        ...
        return this.getMorePose(this.battler()._ras.poseSuffix);
...
Game_CharacterBase.prototype.setVictoryPose = function () {
    if (this.battlerPoses()) {
        this._poses.idle[1] = this._poses.idle[2];
        this._stepAnime = true;
        return this.getMorePose("victory");
...
Game_CharacterBase.prototype.setCastingPose = function () {
    if (this.battlerPoses()) {
        this._poses.idle[1] = this._poses.idle[2];
        this._stepAnime = true;
        return this.getMorePose("casting");
        //$gameScreen.erasePicture(50);//图片
...
/*    if (this.battlerPoses()) {
        this._poses.idle[1] = this._poses.idle[2];
        this._stepAnime = true;
        return this.getMorePose("attacking");
...
Game_CharacterBase.prototype.setGuardingPose = function () {
    if (this.battler()) {
        return this.getMorePose(this.battler()._ras.guard.poseSuffix);
...
Game_Character.prototype.setCharacterFrames = function () {
    ...
    var frames = ww.More_MOG_CharPoses.readGetFrame(this._characterName);

    this._frames = ww.More_MOG_CharPoses.marge(this._frames, frames);
    //console.log(this._frames)
    if (frames) {
        this._frames.enabled = true;
        this._frames.index = 0;
    }
    if (this._frames.enabled) {
        this._pattern = 0;
    }
...
Game_Character.prototype.setRealCharName = function () {
    ...
    this._originalName.name = this._characterName;
    this._originalName.index = this._characterIndex;
    if (this._originalName.name === "") {
        this._poses.enabled = false;
    }
    if (this._poses.enabled && this._characterName != "") {
        this.preCachePoses();
    }
};
...
Game_Character.prototype.preCachePoses = function () {
    ...
    var _cachePoses = [];
    if (this._poses.dash[0]) {
        _cachePoses[0] = ImageManager.loadCharacter(this.getMorePose("dash"));
    }
    if (this._poses.jump[0]) {
        _cachePoses[1] = ImageManager.loadCharacter(this.getMorePose("jump"));
    }
    if (this._poses.idle[0]) {
        _cachePoses[2] = ImageManager.loadCharacter(this.getMorePose("idle"));
    }
    if (Imported.MOG_PickupThrow && !this._eventId) {
        _cachePoses[3] = ImageManager.loadCharacter(this.getMorePose("pick"));
    }
};
...
Game_CharacterBase.prototype.needUpdatePoses = function () {
    if ($gameSystem._chaPoses && !$gameSystem._chaPoses[0]) {
        return false;
    }
    if (!this._poses) {
        return false;
    }
    if (!this._poses.enabled) {
        return false;
    }
...
Game_CharacterBase.prototype.update = function () {
    ...
    if (this.needRefreshPoses()) {
        this.refreshPoses();
    }
    if (this.needUpdatePoses()) {
        this.updatePoses();
    }
    if (this.needRefreshPosesInterpreter()) {
        this.refreshPosesInterpreter();
    }
};
...
Game_CharacterBase.prototype.needRefreshPoses = function () {
    if (this.battler() && this.battler()._originalName.refresh) {
        return true;
    }
    return false;
};
...
Game_CharacterBase.prototype.needRefreshPosesInterpreter = function () {
    ...
    if (this._poses.interpreter != $gameTemp._chaPosesEVRunning) {
        return true;
    }
    if ($gameSystem._chaPoses[0] != $gameSystem._chaPoses[1]) {
        return true;
    }
    return false;
};
...
Game_CharacterBase.prototype.needSetOriginalName = function () {
    if (this._characterName === this._originalName.name) {
        return false;
    }
    if (this._characterName === this.getMorePose("dash")) {
        // if (this._characterName === this.getMorePose(dash) this._originalName.name + "_dash") {
        return true;
    }
    if (this._characterName === this.getMorePose("jump")) {
        //if (this._characterName === this._originalName.name + "_jump") {
        return true;
    }
    if (this._characterName === this.getMorePose("idle")) {
        //if (this._characterName === this._originalName.name + "_") {
        return true;
    }
    return false;
};
...
Game_CharacterBase.prototype.refreshNewPose = function () {
    ...
    $gameTemp.reserveCommonEvent(50);
};
...
Game_CharacterBase.prototype.updateSetPose = function () {
    ...
    if (this._poses.actionMode[0] != this._poses.actionMode[1]) {
        this.refreshActionMode();
    }
    if (charName != this._characterName) {
        this.refreshNewPose();
    }
};
...
//==============================
var _mog_charPose_charBase_updateAnimationCount =
...
// * Set Character
//==============================
var _mog_charPose_SprtChar_setCharacter =
...
Sprite_Character.prototype.needCachePoses = function () {
    ...
    if (!this._character._poses) {
        return false;
    }
    if (!this._character._poses.enabled) {
        return false;
    }
    return true;
};
...
Sprite_Character.prototype.isCharImageChanged = function () {
    ...
    if (this._bitmapCharName !== this._character.characterName()) {
        return true;
    }
    return false;
};
...
Sprite_Character.prototype.setBitmapCache = function () {
    ...
    if (!this._character.characterName()) {
        ...
        this._cacheSprite.bitmap = ImageManager.loadCharacter(
            this._bitmapCharName
        );
    }
};
...
// * update Bitmap
//==============================
var _mog_charPoses_sprChar_updateBitmap =
    Sprite_Character.prototype.updateBitmap;
Sprite_Character.prototype.updateBitmap = function () {
    if (this.isCharImageChanged()) {
        this.setBitmapCache();
    }
    ...
    if (this._character._frames.refresh) {
        this._character.setCharacterFrames();
    }
};
...
//==============================
// * pattern Heigt     /修改于 2021/2/6 1：37
//==============================
...
Sprite_Character.prototype.updateCharPosesPosition = function () {
    if (
        this._character.direction() === 4 ||
        this._character.direction() === 6
    ) {
        var ex =
            this._character.direction() === 6
                ? this._character._frames.x
                : -this._character._frames.x;
    } else {
        var ex = 0;
    }
...
Sprite_Animation.prototype.update = function () {
    ...
    if (this._target && this._target._character) {
        this.updateCharPosesPositionAnimation(this._target._character);
    }
};
...
//==============================
Sprite_Animation.prototype.updateCharPosesPositionAnimation = function (
    character
) {
...
Game_CharacterBase.prototype.setDownPose = function () {
    ...
    return this.getMorePose("down"); //this._originalName.name + "_down";
};
...
Game_CharacterBase.prototype.moveDiagonally = function (horz, vert) {
    if (this.isJumpingE()) {
        ...
        ww_JumpPt._Game_CharacterBase_prototype_moveDiagonally.call(
            this,
            horz,
            vert
        );
