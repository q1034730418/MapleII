 ...
 * 事件穿透：
 *   (1.事件可以拥有多个不同的穿透标签。
 *   (2.根据穿透关系，可以设置敌人不能穿过的墙 或 玩家不能进入的NPC区域。
 *   (3.例如，玩家有个标签："穿透玩家"。其特点为：
 *      只穿过玩家，但是不能穿过其他事件、墙壁、道路障碍等的事件。
 *   (4.如果你想做事件A能穿透B和C，但是BC之间并不能穿透的效果。
 *      那么你需要设置AB为一个标签，AC为另一个标签。
 *   (5.该插件放在任意位置都不会出bug，只是如果它放在跳跃插件的后面时，
...
//			  eventsXyNt + isNormalPriority 【穿透性 + 优先级 同时满足】才阻塞。 
//		
//		★其它说明细节：
//			1.关于同类穿透：rmmv的事件判定比较绕，因为专门区分了 事件、玩家、载具 的关系，以及地图本身的穿透性问题。
