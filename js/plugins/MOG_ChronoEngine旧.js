 ...
 * =============================================================================
 *
 * Tool Id : X    或者：<toolId: x>   注意不可两个备注重复使用
...
Game_Temp.prototype.clearChronoCommand = function () {
    if (this._chronoCom) {
        if ($gameChrono.canStartTurnCN($gameChrono.actor())) {
            $gameChrono.prepareTurnCN($gameChrono.actor(), $gameChrono.comUser()[0])
        };
    }
    this._chronoCom = {};
    ...
    if ($gameChrono && $gameChrono.win()) {
        $gameChrono.win().clearAllWindows()
    };
};
...
Game_Battler.prototype.checkActionID = function (item) {
    ...
    if (item.meta.toolId) actionID = item.meta.toolId;
    return actionID;
};
...
Game_Battler.prototype.toolSysInitBattler = function () {
	...
	this._ras.deadVariableIncrease = {};
...
Game_Battler.prototype.needUpdateAtbC = function () {
    ...
    if ($gameSystem._chronoMode.waitD > 0) {
        return false
    };
    if ($gameSystem._chronoMode.wait) {
        return false
    };
...
Game_Battler.prototype.loadToolSysNotes = function () {
    this.notetags().forEach(function (note) {
        ...
        if (note_data[0].toLowerCase() == "dead variable id") {
			var note_data_value = note_data[1].split(',')
            this._ras.deadVariableID.push(Number(note_data_value[0]));
			this._ras.deadVariableIncrease[Number(note_data_value[0])]=Number(note_data_value[1]);
        };
...
Game_Map.prototype.setup = function (mapId) {
    ...
    if (!$gameSystem._toolsData) {
        this.dataMapToolClear()
    };
...
Game_Map.prototype.loadHookShotData = function () {
    ...
    for (var i = 0; i < $gamePlayer._followers._data.length; i++) {
        ...
        if (char) {
            char.setDirection($gamePlayer._direction)
        }
    };
};
...
Game_Map.prototype.addToolsOnMap = function (tool_id) {
    ...
    if (enable) {
        $gameSystem._toolsOnMap.push(tool_id)
    };
};
...
Game_Map.prototype.clearBattlersOnMap = function () {
    ...
    for (var i = 0; i < followers.length; i++) {
        ...
        if (character && character.battler()) {
...
Game_Map.prototype.removeRasEffectEvents = function (ev) {
    if (Imported.MOG_PickupThrow && ev._throw.enabled) {
        ...
        if (target._pickup.pose) {
            target._characterName = target._pickup.originalName
        };
    };
...
Game_Map.prototype.needClearActionEvent = function (event) {
    if (event._tool.hookshot.enabled) {
        return true
    };
    if (event._tool.boomerang[0]) {
        return true
    };
    return false;
};
...
Game_Map.prototype.getToolsData = function () {
    ...
    var mapId = Number(Moghunter.toolSys_MapID);
    if (!$dataMapTool || !$dataMapTool.events) {
        return
    };
...
Game_Map.prototype.isBattlerOnScreen = function (character, range, ignoreDead) {
    ...
    if (character.battler().isEnemy()) {
        if (!ignoreDead && character.battler().isDead()) {
            return false
        };
        if (character._chrono.escaped) {
            return false
        };
    };
    if (character._tool.enabled) {
        return false
    };
    if (range) {
        if (character.screenX() < this.dspX1()) {
            return false
        };
        if (character.screenX() > this.dspX2()) {
            return false
        };
        if (character.screenY() < this.dspY1()) {
            return false
        };
        if (character.screenY() > this.dspY2()) {
            return false
        };
    };
    return true;
    ...
    for (var i = 0; i < events.length; i++) {
        ...
        if (event.isStarting()) {
            ...
            if (event._tool && event._tool.enabled) {
                this._interpreter.setupToolEvent(event)
            };
            return true;
        }
    }
...
Game_CharacterBase.prototype.saveOrgParameters = function () {
    ...
    if (Imported.MOG_CharPoses) {
        this._chrono.orgoriginalName = this._originalName.name
    };
...
Game_CharacterBase.prototype.canTouchTarget = function (character) {
    ...
    if (!character._user.isPlayer) {
        return false
    };
    if (!character.battler()) {
        return false
    };
    if (character.isKnockbacking()) {
        return false
    };
...
Game_CharacterBase.prototype.clearActing = function (skipCast) {
    ...
    if (this._user.actionTimes[1] > 0) {
        ...
        if (this._user.chainAction[0] > 0) {
            this.executeChainAction()
        };
    };
...
Game_CharacterBase.prototype.canMoveABSBase = function () {
    ...
    if ($gameTemp._chrono.moveWait > 0) {
        return false
    };
    if ($gameTemp._chaPosesEVRunning) {
        return false
    ...
    if (this._user.collapse[0]) {
        return false
    };
    if (this._user.hookshotTool) {
        return false
    };
...
Game_Player.prototype.guardIsUsable = function () {
    ...
    if (!this.battler()._ras.guard.enabled) {
        return false
    };
    return true
};
...
Game_CharacterBase.prototype.isActing = function () {
    ...
    if (this.battler()._ras.hookshotUser[0]) {
        return true
    };
    if (this.battler()._ras.poseDuration > 0) {
        return true
    };
    return false
};
...
Game_CharacterBase.prototype.isKnockbacking = function () {
    ...
    if (this.battler()._ras.knockback[1] > 0) {
        return true
    };
    if (this.battler().isDead()) {
        return true
...
Game_CharacterBase.prototype.canKnockback = function (target) {
    ...
    if (target.battler()._ras.hookshotUser[0]) {
        return false
    };
    if (target.battler()._ras.invunerable) {
        return false
    };
    if (target.battler()._ras.superGuard) {
        return false
    };
...
Game_CharacterBase.prototype.canExecuteAction = function (toolID) {
    ...
    if (this.isRequiredCast(toolID)) {
        this.prepareCast(toolID);
        return false
    };
    if (this.isAutoTarget(toolID)) {
        this.executeAutoTarget(toolID);
        return false
    };
    return true;
};
...
Game_CharacterBase.prototype.act = function (toolID) {
    if (!this.canExecuteAction(toolID)) {
        return
    };
    if (!$gameSystem._eventDataTool) {
        $gameSystem._eventDataTool = []
    };
...
Game_CharacterBase.prototype.isRequiredCast = function (toolID) {
    ...
    if (this.battler()._ras.cast.item) {
        return false
    };
    var action = $gameMap.toolEvent(toolID);
    if (!action) {
    ...
    if (Math.abs(action.item().speed) === 0) {
        return false
    };
    return true;
};
...
Game_CharacterBase.prototype.setDeadVariable = function (battler) {
    for (var i = 0; i < battler._ras.deadVariableID.length; i++) {
        ...
        if (variableId) {
			...
			var increase = battler._ras.deadVariableIncrease[variableId];
			console.log(increase=="NaN");
			console.log(increase==NaN);
			if(increase>1){
				$gameVariables.setValue(variableId, oldValue + increase);
			}else{
...
Game_CharacterBase.prototype.needStopCharacterUpdate = function () {
    if ($gameTemp._autoTarget.enabled && !$gameSystem.isChronoMode()) {
        return true
    };
    if (this.needStopNonChronoBattlers()) {
        return true
    };
    return false;
};
...
Game_CharacterBase.prototype.update = function () {
    if (this.needStopCharacterUpdate()) {
        return
    };
    _mog_toolSystem_gCharBase_update.call(this);
    if (this._tool) {
        this.updateToolBasic()
    };
};
...
Game_CharacterBase.prototype.updateToolBasic = function () {
    if (this._user.hookshotLock.tool) {
        this.updateHookShotLock()
    };
    if (this.battler()) {
        if (this.needUpdateAutoAction()) {
        ...
        if (!this._tool.enabled) {
            this.updateRasBattler()
        };
    };
    if (this._user.treasure[4] > 0) {
        this._user.treasure[4]--
    };
};
...
Game_CharacterBase.prototype.needUpdateAutoAction = function () {
    ...
    if (this._user.actionTimes[2] === 0) {
        return false
    };
    if (this._user.actionTimes[5] === 0) {
        return false
    };
    return true;
};
...
Game_CharacterBase.prototype.updateRasBattler = function () {
    ...
    if (this.battler()._ras.poseDuration > 0) {
        this.updatePoseDuration()
    };
    if (this.battler()._ras.collisionD > 0) {
        this.battler()._ras.collisionD--
    };
    ...
    if (this.battler()._ras.hookshotUser[1]) {
        this.updateHookshot()
    };
    if (this.battler().isCharging() && !this.isKnockbacking()) {
        this.updateCharging()
    };
    if (this.isCasting() && !$gameSystem._chronoMode.inTurn) {
        this.updateCasting()
    };
    if ($gameSystem.isChronoMode()) {
        this.updateChronoModeChar()
    }
};
...
Game_CharacterBase.prototype.updateCasting = function () {
    if (this.canUpdateCastTime()) {
        this.battler()._ras.cast.duration--
    };
    if (this.battler()._ras.cast.duration === 0) {
        this.executeCast()
    };
    if (this.canUpdateCastAnimation()) {
        this.updateCastAnimation()
    };
};
...
Game_CharacterBase.prototype.canUpdateCastTime = function () {
    ...
    if ($gameSystem._chronoMode.waitMode < 2) {
        ...
        if (!$gameTemp._autoTarget.enabled && $gameTemp._chronoCom.phase != 0) {
            return false
        };
    };
    return true;
...
Game_CharacterBase.prototype.canUpdateCastAnimation = function () {
    if (this.battler()._ras.cast.animationMax === 0) {
        return false
    };
    if (this.battler()._ras.cast.duration < this.battler()._ras.cast.animationMax) {
        return false
    };
    return true;
};
...
Game_CharacterBase.prototype.needStopPatternAction = function () {
    ...
    if (this.battler()._ras.poseLoop) {
        return false
    };
    if (Imported.MOG_CharPoses) {
        if (this._pattern < this._frames.max) {
            return false
        };
    } else {
        if (this._pattern < this._user.poseMaxPattern) {
            return false
        };
    };
    return true
...
Game_Player.prototype.moveByInput = function () {
    if (!this.canMoveABS() || $gameSystem.isChronoMode()) {
        return
    };
...
Game_Player.prototype.commandToolUsable = function () {
    ...
    if (Imported.MOG_PickupThrow) {
        if (this._pickup.enabled) {
            return false
        };
        if (this._pickup.wait > 0) {
            return false
        };
    };
    return true;
...
Game_Player.prototype.isComboAction = function (actionID, type) {
    if (!$gameMap.toolIsExist(actionID)) {
        return false
    };
    var tool = $gameMap.tool(actionID);
    if (tool.combo.id === 0) {
...
Game_Player.prototype.commandAttackUsable = function () {
    if (!$gameSystem._chronoCom.attack) {
        return false
    };
    if (Input.isRepeated(Moghunter.ras_buttonWeapon)) {
        return true
    };
    return false;
};
...
Game_Player.prototype.commandSkillUsable = function () {
    if (!$gameSystem._chronoCom.skill) {
        return false
    };
    if (Input.isTriggered(Moghunter.ras_buttonSkill)) {
        return true
    };
    return false;
};
...
Game_Player.prototype.commandItemUsable = function () {
    if (!$gameSystem._chronoCom.item) {
        return false
    };
    if (Input.isTriggered(Moghunter.ras_buttonItem)) {
        return true
    };
    return false;
};
...
Game_Player.prototype.commandGuardUsable = function () {
    ...
    if (!this.guardIsUsable()) {
        return false
    };
    if (!$gameSystem._chronoCom.shield) {
        return false
    };
    if (Input.isPressed(Moghunter.ras_buttonGuard)) {
        return true
    };
    return false;
};
...
Game_Player.prototype.commandChargeUsable = function () {
    ...
    if (this.battler()._ras.charge.id == 0) {
        return false
    };
    if (Input.isPressed(Moghunter.ras_buttonWeapon)) {
        return true
    };
    return false;
};
...
Game_Event.prototype.start = function () {
    if (this._user.toolCollision.length > 0 && this._trigger != 9) {
        return
    };
...
Game_Event.prototype.canStartMapEvent = function (triggers, normal) {
    if (!this.isTriggerIn(triggers)) {
        return false
    };
    if (this.isNormalPriority() != normal) {
        return false
    };
    if (this.battler()) {
        if (!this._tool.enabled) {
        ...
        if (!this._tool.eventInt) {
            return false
        };
    };
    if (this._user.toolCollision.length > 0) {
        return false
    };
    return true;
};
            ...
            if (l.code === 108) {
                ...
                if (comment[0].toLowerCase() == "enemy_id") {
                    ...
                    if ($dataEnemies[enemyId]) {
                        if (!this._user.battler) {
                            this._user.battler = new Game_Enemy(enemyId, 0, 0)
                        };
                        if (this._poses) {
...
Game_Event.prototype.needStopSelfMovement = function () {
    ...
    if (this._user.treasure[0]) {
        return true
    };
    if (this._chrono.escaped) {
        return true
    };
    if (!this.canMoveABS()) {
        return true
    ...
    if (this.battler() && $gameSystem.isChronoMode()) {
        ...
        if ($gameSystem._chronoMode.waitMode < 2) {
            if ($gameSystem._chronoMode.waitMode === 0 && $gameTemp._chronoCom.user) {
                return true
            };
            if ($gameTemp._autoTarget.enabled) {
                return true
...
Game_Event.prototype.moveTypeRandom = function () {
    if (this.needMoveTowardPlayerCR()) {
        this.moveTypeTowardPlayer();
        return
    };
    if (this.needWalkAround()) {
        this.walkAround();
        return
    };
...
Game_Chrono.prototype.battleStartChrono = function () {
    ...
    for (var i = 0; i < $gameMap.players().length; i++) {
        ...
        if (character._user.isPlayer) {
            $gamePlayer._moveSpeed = $gamePlayer._chrono.premapMoveSpeed
        };
...
Game_Chrono.prototype.setInitialActorsPositionChrono = function () {
    for (var i = 0; i < $gameMap.players().length; i++) {
        ...
        if (i > 0) {
            this.setInitialFollowerPositionChrono(i, character)
        };
...
Game_Chrono.prototype.needUpdateEscapePhase = function () {
    if (!this.escape().enabled) {
        return false
    };
    if ($gameSystem._chronoMode.phase != 7) {
        return false
    };
    return true;
};
...
Game_Chrono.prototype.executeDeadEffect = function () {
    for (var i = 0; i < $gameMap.enemiesF().length; i++) {
        ...
        if (char.battler().isDead() && !char._chrono.deadChecked) {
            if ($gameSystem._chronoMode.phase === 3) {
                if (Imported.MOG_BossHP && char.battler()._bosshp_meter) {
                    this.checkBossHp()
                };
            };
...
Game_Chrono.prototype.updateBasic = function () {
    if ($gameSystem._chronoMode.waitD > 0) {
        $gameSystem._chronoMode.waitD--
    };
    if ($gameTemp._chrono.buttonLag > 0) {
        $gameTemp._chrono.buttonLag--
    };
    if ($gameTemp._chrono.clearCommadPhase) {
        this.clearCommandPhase()
    };
...
Game_Chrono.prototype.updateChronoTurn = function (char, battler) {
    if (this.needUpdateTurnCN(char, battler)) {
        this.updateTurnPhaseCN(char, battler)
    };
};
...
Game_Chrono.prototype.needUpdateTurnCN = function (char, battler) {
    ...
    if (!battler._chrono.action) {
        return false
    };
    if (!battler._chrono.inAction) {
        return false
    };
    if (char.isCasting()) {
        return false
...
Game_Chrono.prototype.makeTargetsCN = function (battler, item) {
    ...
    if (target) {
        battler._chrono.targets[battler._chrono.actionTimes] = target
    }
    return target;
};
...
Game_Chrono.prototype.item = function (battler) {
    if (battler._chrono.action) {
        return battler._chrono.action
    };
    if (battler.currentAction()) {
        return battler.currentAction().item()
    };
    return null;
};
...
Game_Chrono.prototype.isActionIsAvailable = function (char, battler, target, actionID, item) {
    ...
    if (!$gameChrono.isCooperationMeet(item, false)) {
        return false
    };
    return true;
};
...
Game_Chrono.prototype.clearTurnPhase = function (char, battler) {
    battler.atbClearCN();
    if (this.item(battler)) {
        this.clearCooperationSkill(this.item(battler))
    };
    $gameSystem._chronoMode.inTurn = false;
    $gameSystem._chronoMode.inTurnBattler = null
    $gameSystem._chronoMode.battlerActive = null;
...
Game_Chrono.prototype.updateMoveToTargetCN = function (char, battler) {
    if (!char.isMoving() && !char.isJumping()) {
        battler._chrono.actionPhase = 2
    };
};
...
Game_Chrono.prototype.updateChronoATB = function (battler, char) {
    if (!$gameSystem._chronoMode.inTurn && !battler.isDead()) {
        this.updateStatesTurn(battler, char)
    };
    if (!battler.isMaxAtbC()) {
        if (battler.needUpdateAtbC()) {
            battler._chrono.atb += battler.atbSpeedC()
        };
        if (battler.isMaxAtbC()) {
        ...
        if (this.canStartCommandCN(battler)) {
            this.prepareCommandSelectionCN(battler, char)
        };
        if (this.canStartTurnCN(battler)) {
            this.prepareTurnCN(battler, char)
        };
    };
    if (battler._chrono.atb > battler._chrono.maxAtb) {
        battler._chrono.atb = battler._chrono.maxAtb
    };
    if (battler._chrono.atb < 0) {
        battler._chrono.atb = 0
    };
};
...
Game_Chrono.prototype.onStatesTurn = function (battler, char) {
    battler._chrono.statesTurn[0] = 4;
...
Game_Chrono.prototype.needCastActionEnemy = function (battler, char) {
    ...
    if (item.meta.toolId) char.prepareCast(item.meta.toolId);
};
...
Game_Chrono.prototype.isGuardingDirectionTouch = function (attacker, target) {
    ...
    if (attacker._user.diagonal[0]) {
        if (target.direction() === 2) {
            if (attacker._user.diagonal[1] === 9) {
                return true
            };
            if (attacker._user.diagonal[1] === 7) {
                return true
            };
        } else if (target.direction() === 8) {
            if (attacker._user.diagonal[1] === 1) {
                return true
            };
            if (attacker._user.diagonal[1] === 3) {
                return true
            };
        ...
        if (target.direction() === 2 && attacker.direction() === 8) {
            return true
        };
        if (target.direction() === 8 && attacker.direction() === 2) {
            return true
        };
        if (target.direction() === 4 && attacker.direction() === 6) {
            return true
        };
        if (target.direction() === 6 && attacker.direction() === 4) {
            return true
        };
    };
    return false;
...
Game_Chrono.prototype.executeTouchDamage = function (user, target) {
    if (target.battler()._ras.collisionD > 0) {
        return
    };
...
Game_Chrono.prototype.allCoopMembersInBattle = function (item, inTurn) {
    if (this._actorsIdsCop.length === 0) {
        return true
    }
    var actors = 0;
    for (var s = 0; s < $gameMap.players().length; s++) {
        ...
        for (var i = 0; i < this._actorsIdsCop.length; i++) {
            ...
            if (battlerID === actorID) {
                actors++
            };
        };
    };
    if (actors != this._actorsIdsCop.length) {
        return false
    };
    return true;
};
...
Game_Chrono.prototype.isCooperationMeet = function (item, inTurn) {
    ...
    for (var i = 0; i < cpmembers.length; i++) {
        ...
        if (actor.restriction() > 0) {
            enable = false
        };
        if (inTurn && actor._chrono.action) {
            enable = false
        };
    };
    if (!this.allCoopMembersInBattle()) {
        enable = false
    };
...
Game_Chrono.prototype.setCooperationSkill = function (item) {
    ...
    for (var i = 0; i < members.length; i++) {
        var char = members[i];
        var battler = char.battler();
        if (char.isRequiredCast(actionId)) {
            char.prepareCast(actionId)
        };
...
Game_Event.prototype.canStartCrBattle = function () {
    if (!$gameSystem._chronoMode.enabled) {
        return false
    };
    if ($gameSystem._chronoMode.phase != 0) {
        return false
    };
    if (!this.battler()) {
        return false
    ...
    if ($gamePlayer._user.hookshotTool) {
        return false
    };
    if (!$gameParty.leader()) {
        return false
    };
    if ($gamePlayer.isActing()) {
        return false
    };
    if ($gamePlayer.isKnockbacking()) {
        return false
    };
...
Game_Event.prototype.prepareBattle = function () {
    $gameMap.targetsOnScreen();
    if ($gameMap.enemiesF().length > 0) {
        $gameChrono.prepareChronoBattle()
    };
};
...
Game_Action.prototype.executeDamageCN = function (target, value) {
    ...
    if (value > 0 && target._ras.guard.chrono) {
...
ToolEvent.prototype.initialize = function (mapId, eventId, user, getdata) {
    ...
    if (!this._preLoad) {
        this._tool.user.battler()._ras.tool = this
    };
    this.checkToolNotes();
...
ToolEvent.prototype.toolUsable = function () {
    ...
    if (this.user()._tool.enabled) {
        return true
    };
    if (!this.canUseUniqueTools()) {
        return false
    };
...
ToolEvent.prototype.canPayActionCost = function () {
    if (!this.canPayItemCost()) {
        return false
    };
    if (!this.canPayMPCost()) {
        return false
    };
    if (!this.canPayTPCost()) {
        return false
    };
    if (!this.canPayCTCost()) {
        return false
    };
    return true;
};
...
ToolEvent.prototype.canUseUniqueTools = function () {
    if (!this._tool.unique) {
        return true
    };
    var enable = true;
    for (var i = 0; i < $gameSystem._toolsOnMap.length; i++) {
        var toolMapID = $gameSystem._toolsOnMap[i];
        if (this._tool.id === toolMapID) {
            enable = false
        };
    };
    return enable;
...
ToolEvent.prototype.payCost = function () {
    ...
    if (this.user()._user.spcShoot[0]) {
        return
    };
    if (this._payCost.item) {
        this.payItemCost()
    };
    if (this._payCost.mp) {
        this.payMPCost()
...
ToolEvent.prototype.canPayItemCost = function () {
    ...
    if (this.user().battler()) {
        if (this.user().battler().isEnemy()) {
            return true
        };
        if (!$gameParty.hasItem($dataItems[this._tool.itemCost])) {
            return false
        };
        this._payCost.item = item;
        return true
    };
    if (this.user()._user.isEvent) {
        return true
    };
    if (!$gameParty.hasItem($dataItems[this._tool.itemCost])) {
        return false
...
ToolEvent.prototype.canPayMPCost = function () {
    ...
    if (!skillCost || skillCost <= 0) {
        return true
    };
    if (this.user().battler()) {
        if (this.user().battler()._mp < skillCost) {
            return false
        };
...
ToolEvent.prototype.canPayTPCost = function () {
    ...
    if (this.user().battler()) {
        if (this.user().battler()._tp < skillCost) {
            return false
        };
...
ToolEvent.prototype.startTool = function () {
    ...
    if (this._tool.projectile && $gameSystem.isChronoMode()) {
        this.setProjectileEffect()
    };
    ...
    if (this._tool.zoomAct) {
        this.user().charZoomAct(true, false)
    };
};
...
ToolEvent.prototype.setAngleSprite = function () {
    ...
    if (angle != 0) {
        this._tool.diagonalAngle = angle + Math.PI / 2.0
    };
};
...
ToolEvent.prototype.canSetAngleSprite = function () {
    ...
    if (this._tool.diagonalAngle === 0) {
        return false
    };
    if (!this._tool.diagonal) {
        return false
    };
    return true;
};
...
ToolEvent.prototype.updateCollision = function () {
    for (var i = 0; i < this.targets().length; i++) {
        ...
        if (target && this.canCollide(target)) {
            this.executeCollision(target)
        };
    };
};
...
ToolEvent.prototype.isAlreadyCollided = function (target) {
    ...
    for (var i = 0; i < this._collided.length; i++) {
        if (this._collided[i] === target) {
            coll = true
        };
    };
    if (!coll) {
        this._collided.push(target)
    };
    return coll
};
...
ToolEvent.prototype.inRange = function (dx, dy) {
    ...
    if (mode === 0) {
        ...
        if (this._direction === 2) {
            if (Math.abs(dx) + Math.abs(dy) <= range && dy >= 0) {
                return true
            };
        } else if (this._direction === 8) {
            if (Math.abs(dx) + Math.abs(dy) <= range && dy <= 0) {
            ...
            if (Math.abs(dx) + Math.abs(dy) <= range && dx >= 0) {
                return true
            };
        } else if (this._direction === 4) {
            if (Math.abs(dx) + Math.abs(dy) <= range && dx <= 0) {
        ...
        if (this._direction === 2) {
            if (Math.abs(dx) <= range && dy >= 0 && Math.abs(dy) <= range) {
                return true
            };
        } else if (this._direction === 4) {
            if (Math.abs(dx) <= range && dx <= 0 && Math.abs(dy) <= range) {
                return true
            };
        } else if (this._direction === 6) {
            if (Math.abs(dx) <= range && dx >= 0 && Math.abs(dy) <= range) {
        ...
        if (this._direction === 2) {
            if (Math.abs(dx) <= range && dy == 0) {
                return true
            };
        } else if (this._direction === 4) {
            if (Math.abs(dy) <= range && dx == 0) {
                return true
            };
        } else if (this._direction === 6) {
            if (Math.abs(dy) <= range && dx == 0) {
...
ToolEvent.prototype.canCollide = function (target) {
    ...
    if (this._tool.removeSprite) {
        return false
    };
    if (target === this) {
        return false
    };
    if (!target._user.collision) {
        return false
    };
    if (target.needStopNonChronoBattlers()) {
        return false
    };
    if ($gameTemp._hookshootPreData.user && $gameTemp._hookshootPreData.user === this.user()) {
        return false;
    };
    if (target._characterName == '') {
        return false
    };
    if (target.battler()) {
        if (target.battler()._ras.disableCollision) {
            return false
        };
        ...
        if (target.battler()._ras.hookshotUser[1]) {
            return false
        };
    };
    if (target._tool.enabled) {
        return false
    };
    if (target._erased) {
        return false
    };
    if (target._user.treasure[4] > 0) {
        return false
    };
    if (target._user.hookshotTool) {
        return false
    };
    if (target._transparent) {
        return false
    };
    ...
    if (!this._tool.multihit) {
        if (this.isAlreadyCollided(target)) {
            return false
        };
    };
    ...
    if (target._user.isPlayer) {
        ...
        if (!target._user.isLeader && !target.isVisible()) {
            return false
        };
    };
    ...
    if (this.isCollisionEvent(target)) {
        return true
    };
    if (this.isHookshotEvent(target)) {
        return true
    };
    return false;
};
...
ToolEvent.prototype.isTreasure = function (target) {
    if (!target._user.treasure[0]) {
        return false
    };
    if (!this.user()._user.isPlayer) {
        return false
    };
    if (!this._tool.getTreasure) {
        return false
    };
    return true;
};
...
ToolEvent.prototype.checkWeapon = function (battler) {
    ...
    if (this._tool.id != battler.attackSkillId()) {
        return false
    };
    if (!battler.equips()[0]) {
        return false
...
ToolEvent.prototype.collisionGuard = function (char, battler) {
    ...
    var animationID = Moghunter.ras_guardAnimationID;
    if (animationID) {
        char.requestAnimation(animationID)
    };
...
ToolEvent.prototype.setReverseDiagonal = function () {
    ...
    if (this._user.diagonal[1] === 9) {
        return 1
    };
    if (this._user.diagonal[1] === 3) {
        return 7
    };
    if (this._user.diagonal[1] === 7) {
        return 3
    };
    return 0;
};
...
ToolEvent.prototype.targetCanGuard = function (char, battler) {
    ...
    if (this._tool.ignoreGuard) {
        return false
    }
    if (!char.isGuarding()) {
        return false
    };
    if (!this.isGuardingDirection(char)) {
        return false
    };
    return true;
};
...
ToolEvent.prototype.isGuardingDirection = function (char) {
    if (this._user.diagonal[0]) {
        ...
        if (char.direction() === 2 && this.direction() === 8) {
            return true
        };
        if (char.direction() === 8 && this.direction() === 2) {
            return true
        };
        if (char.direction() === 4 && this.direction() === 6) {
            return true
        };
        if (char.direction() === 6 && this.direction() === 4) {
            return true
        };
    };
    return false;
...
ToolEvent.prototype.canCollideBattler = function (target) {
    ...
    if (target.battler()._chrono.defeated[1]) {
        return false
    };
    if (target.battler()._chrono.escaped) {
        return false
    };
    if (!this.item()) {
        return false
    ...
    if (!this.inScope(this.user().battler(), target.battler())) {
        return false
    };
    return true
};
...
ToolEvent.prototype.availableTargets = function (char, targetMode) {
    ...
    if (!$gameMap.isBattlerOnScreen(char, true, true)) {
        return false
    };
    if (targetMode === 0) {
        ...
        if (!this.collidedXY(char)) {
            return false
        };
    };
    if (!this.canCollideBattler(char)) {
        return false
    };
    return true;
};
...
ToolEvent.prototype.isInvunerableAction = function (target, battler) {
    if (battler._ras.invunerable && this.user() != target) {
        return true
    };
...
ToolEvent.prototype.item = function () {
    if (this._tool.skill) {
        return this._tool.skill
    };
    if (this._tool.item) {
        return this._tool.item
    };
    return null;
};
...
ToolEvent.prototype.isOpponent = function (user, target) {
    if (user.isEnemy() && target.isActor()) {
        return true
    };
    if (user.isActor() && target.isEnemy()) {
        return true
    };
    return false;
};
...
ToolEvent.prototype.deadEffectEnemy = function (char, battler) {
    if (!$gameSystem.isChronoMode()) {
        if (Imported.MOG_BossHP && battler._bosshp_meter) {
            $gameChrono.checkBossHp()
        };
        if (this.user().battler().isActor()) {
            this.gainExpGold(char, battler)
        };
...
ToolEvent.prototype.needRefreshTargets = function () {
    ...
    if (!$gameTemp._chronoCom.user[1]) {
        return false
    };
    if (!$gameTemp._chronoCom.skill) {
        return false
    };
    return true;
};
...
ToolEvent.prototype.collisionAfterHitBattler = function (char, battler, oldHP) {
    ...
    if (battler.isDead()) {
        this.afterDead(char, battler)
    };
    this.collisionSprite(char, oldHP);
    if (this.needForceClearCommand()) {
        this.forceClearCommand()
    };
    if (this.needChainHitAction(char, battler)) {
        this.setChainHitAction(char, battler)
    };
    if (!battler.canMove()) {
        char._stepAnime = false
    };
...
ToolEvent.prototype.needClearCooperationSkill = function (char, battler) {
    ...
    if (!$gameChrono.isIncludeCooperationSkill(battler)) {
        return false
    };
    if ($gameChrono.isCooperationMeet(battler._chrono.action, false)) {
        return false
    };
    return true;
};
...
ToolEvent.prototype.isHookshotEvent = function (target) {
    ...
    if (this.isCollidedWithCharacters(ex, ey)) {
        return false
    }
...
ToolEvent.prototype.executeCollision = function (target) {
    if (this.isCollisionEvent(target)) {
        this.collisionTool(target)
    };
    if (this.isHookshotEvent(target)) {
        this.collisionHookshot(target)
    };
    if (target === this.user()) {
        this.collisionUser(target)
    };
    if (!this.isPiercing()) {
        this.durationOne()
    };
    if (target.battler()) {
        this.setInvunerableDuration(target)
    };
};
...
ToolEvent.prototype.updateToolSystem = function () {
    if (this.needRemoveForcedMoveTool()) {
        this.removeForcedMoveTool()
    };
    if (this.needUpdateToolDuration()) {
        this.updateToolDuration()
    };
    if (this._tool.wait > 0) {
        this.updateToolWait()
    };
...
ToolCursor.prototype.battlers = function () {
    if ($gameTemp._chronoCom.targets.length > 0) {
...
ToolCursor.prototype.chronoPhaseSelection = function () {
    ...
    if (item) {
        $gameChrono.setCooperationSkill(item)
    };
...
ToolCursor.prototype.nextIndex = function (value) {
    ...
    this._lagTime = 3;
    if (this._index < 0) {
        this._index = this.battlers().length - 1
    };
    if (this._index >= this.battlers().length) {
        this._index = 0
    };
};
...
ToolCursor.prototype.needRefreshCursor = function () {
    if ($gameTemp._autoTarget.enabled != this._active) {
        return true
    };
    if ($gameTemp._autoTarget.refresh) {
        return true
    };
    if (this._hpGauge && this._target) {
        if (this._hpGauge.hp != this._target.battler().hp) {
            return true
        };
        if (this._hpGauge.mhp != this._target.battler().mhp) {
            return true
        };
    };
    return false;
...
Game_Character.prototype.moveDiagonally = function (horz, vert) {
    ...
    if (!this.isMovementSucceeded() && !this._tool.enabled) {
        ...
        this.getDiagonalDirection(horz, vert)
    }
};
...
Game_CharacterBase.prototype.moveDiagonallyAfter = function (horz, vert) {
    ...
    if (this.needForceDiagonalDirection()) {
        this.forceDiagonalbyInput(horz, vert)
    };
};
...
Game_CharacterBase.prototype.isDashing = function () {
    if (this.battler() && this.battler()._ras.hookshotUser[0]) {
        return false
    };
...
Game_Character.prototype.moveForward = function () {
    if (this.needMoveFowardDiagonal()) {
...
Game_Character.prototype.needMoveFowardDiagonal = function () {
    ...
    if (this._tool.enabled && !this._tool.diagonal) {
        return false
    };
    if (!this._user.diagonal[0]) {
        return false
...
Game_Character.prototype.processMoveCommand = function (command) {
    if (this.needProcessMoveCoomandTool()) {
        this.processMoveCommandTool(command);
        return
    };
...
Game_Character.prototype.needRemoveForcedMoveTool = function () {
    ...
    if (this.screenX() < this._tool.user.screenX() - 24) {
        return false
    };
    if (this.screenX() > this._tool.user.screenX() + 24) {
        return false
    };
    if (this.screenY() < this._tool.user.screenY() - 24) {
        return false
    };
    if (this.screenY() > this._tool.user.screenY() + 24) {
        return false
...
Game_Character.prototype.moveTowardCharacter = function (character) {
    if (this.needMoveTowardDiagonal(character)) {
        this.moveTowardCharacterDiagonal(character);
        return
    };
...
Game_Character.prototype.turnTowardCharacter = function (character) {
    if (this.needMoveTowardDiagonal(character)) {
        this.turnTowardDiagonal(character);
        return
    };
...
Game_Character.prototype.getTargetCN = function () {
    if ($gameSystem.isChronoMode()) {
        return this.battler()._chrono.targets[0]
    };
    if (this.battler().isEnemy()) {
        return $gamePlayer
    }
    ...
    if (!through && !this._tool.active && this.isCollidedWithBattlers(absX, absY)) {
        if (this._direction === 4 || this._direction === 6) {
            for (var i = 0; i < disX2; i++) {
                ...
                var absX = pos[0];
                var absY = pos[1];
                if (!through && !this.isCollidedWithBattlers(absX, absY)) {
                    break
                };
            };
        } else {
...
Game_CharacterBase.prototype.needWalkAround = function () {
    ...
    if (disX >= this._user.walkAround[3]) {
        return true
    };
    if (disY >= this._user.walkAround[3]) {
        return true
    };
    return false;
};
...
Game_CharacterBase.prototype.teleportOrg = function () {
    if (this._user.orgX === 0 && this._user.orgY === 0) {
        return
    };
...
Game_CharacterBase.prototype.updateAnimation = function () {
    if (this.isActing() && this._pattern >= this.maxPatternABS()) {
        return
    }
...
Game_Player.prototype.getInputDirection = function () {
    if ($gameSystem._chronoCom.diagonal) {
        return Input.dir8
    };
    return Input.dir4;
};
...
Window_ToolSkill.prototype.includes = function (item) {
    ...
    if (item.meta.toolId) {
        this._toolID[this.itemIndex] = item.meta.toolId;
        this.itemIndex++;
        enable = true;
    }
...
Game_Chrono.prototype.nextActor = function (value) {
    ...
    for (var i = 0; i < $gameMap.players().length; i++) {
        ...
        if (this.isUsableActor(actor.battler())) {
            avaliableActors.push(actor)
        };
    };
...
Game_Chrono.prototype.setCastAction = function () {
    if (this.toolAction()) {
        if (this.comUser()[0].isRequiredCast(this.toolAction().id)) {
            this.comUser()[0].prepareCast(this.toolAction().id)
        };
    };
};
...
Game_Chrono.prototype.getToolId = function (item) {
    ...
    if (item) {
        ...
        if (item.meta.toolId) toolID = item.meta.toolId;
    };
    return toolID;
};
...
Game_Chrono.prototype.needSelection = function (item) {
    ...
    if (!scopeSelection.contains(item.scope)) {
        return false
    };
    if (tool.projectile) {
        return true
...
Game_Chrono.prototype.cancelTurnCom = function () {
    ...
    for (var i = 0; i < $gameMap.players().length; i++) {
        var char = $gameMap.players()[i];
        var battler = char.battler();
        if (battler != preActor && this.canStartCommandCN(battler)) {
            this.prepareCommandSelectionCN(battler, char)
        };
    };
    $gameTemp._chrono.buttonLag = 5;
...
Game_Chrono.prototype.nextComIndex = function (value) {
    ...
    SoundManager.playCursor();
    if ($gameTemp._chronoCom.index < 0) {
        $gameTemp._chronoCom.index = 3
    };
    if ($gameTemp._chronoCom.index > 3) {
        $gameTemp._chronoCom.index = 0
    };
};
...
ChronoWindows.prototype.onSkillOk = function () {
    ...
    if ($gameTemp._chronoCom.targets.length === 0 && $gameChrono.actor()._chrono.targets.length === 0) {
        if (!$gameChrono._selectionOK) {
            $gameChrono.commandSkill(true)
        };
    };
};
...
ChronoWindows.prototype.isTouchCommand = function (x, y) {
    ...
    if (x > this.comLmRight()) {
        return false
    };
    if (y > this.comLmBelow()) {
        return false
    };
    if (y < this.comLmAbove()) {
        return false
    };
    return true;
};
...
ChronoWindows.prototype.update = function () {
    Sprite.prototype.update.call(this);
    if (this._comImg2.isReady()) {
        this.updateCommand()
    };
    if ($gameTemp._chrono.refreshWindow) {
        this.refreshWindows()
    };
};
...
ChronoWindows.prototype.updateCommand = function () {
    ...
    if ($gameTemp._chronoCom.index != this._command2.index) {
        this.refreshComIndex()
    };
    if (this._command1.scale.x > 1.00) {
        this._command1.scale.x -= 0.1;
        if (this._command1.scale.x <= 1.00) {
            this._command1.scale.x = 1.00
        }
    };
    if (!this._command1.visible) {
        this._command1.scale.x = 1.5
    };
    ...
    for (var i = 0; i < 2; i++) {
        if (this._comArrow[i].refresh) {
            if (this._comImg3.isReady()) {
                this.comArrowRefresh(this._comArrow[i], i)
            };
            return;
        };
        ...
        if (this._comArrow[i].slide[0]) {
            this.updateArrowSlide(this._comArrow[i])
        };
        if (i === 0) {
...
ChronoWindows.prototype.isTouchArrowCommand = function (index, x, y) {
    if (x < this.arrowLmLeft(index)) {
        return false
    };
    if (x > this.arrowLmRight(index)) {
        return false
    };
    if (y > this.arrowLmBelow(index)) {
        return false
    };
    if (y < this.arrowLmAbove(index)) {
        return false
    };
    return true;
};
...
Window_ChronoSkill.prototype.isEnabled = function (item) {
    ...
    if (!this._actor.canUse(item)) {
        return false
    };
    if (!$gameChrono.isCooperationMeet(item, true)) {
        return false
    };
    return true;
};
...
Game_Interpreter.prototype.user = function () {
    if (this.tool() && this.tool().user()) {
        return this.tool().user()
    };
    return $gamePlayer;
};
...
Spriteset_Map.prototype.initialize = function () {
    if ($gameParty.leader() && $gameParty.leader().isDead()) {
        $gameChrono.selectNextActor()
    };
...
Spriteset_Map.prototype.updateChronoSprites = function () {
    if (!this._chronoField) {
        this.createChronoField()
    };
    this.updateChronoFieldVisible();
    if (this._chronoSpriteCache && $gameSystem._chronoMode.needCreateSprites) {
        this.createChronoSprites()
    };
    if ($gameSystem._chronoMode.needRemoveSprites) {
        this.removeChronoSprites()
    };
    if (!this._chronoSpriteCache) {
        this.cacheSpriteChronoCache()
    };
    if ($gameSystem.isChronoMode()) {
        this.updateChronoSpritesActive()
    };
    if (!this._chronoPhase) {
        this.createChronoPhase()
    };
    if (!this._resultCR) {
        this.createResultCR()
    };
...
Spriteset_Map.prototype.removeChronoSprites = function () {
    ...
    if (Imported.MOG_BossHP) {
        $gameTemp._forceRemoveBossHud = true
    };
    if (this._atbGaugeCR) {
        this._chronoField.removeChild(this._atbGaugeCR)
    };
    if (this._chronoWindow) {
        this._hudField.removeChild(this._chronoWindow)
    };
    if (this._escape1CR) {
...
Spriteset_Map.prototype.updateResultCR = function () {
    if ($gameTemp._chrono.victory[1]) {
        this.refreshResultCR()
    };
    ...
    if ($gameTemp._chrono.victory[0]) {
        if (this._resultCR.x < this._resultCR.org[0]) {
            this._resultCR.x += 5
        };
        this._resultCR.opacity += 5;
    } else {
        this._resultCR.opacity -= 10;
        if (this._resultCR.opacity > 0) {
            this._resultCR.x += 5
        };
    };
...
Spriteset_Map.prototype.cacheSpriteChronoCache = function () {
    if (Imported.MOG_BattleHud && !this._bhudImgCache) {
        this.preCacheImagesBHud()
    };
...
Spriteset_Map.prototype.preCacheImagesBHud = function () {
    ...
    if (String(Moghunter.bhud_layoverlay_visible) == "true") {
        this._bhudImgCache.push(ImageManager.loadBHud("Layout2"))
    };
    ...
    if (String(Moghunter.bhud_hp_meter_visible) == "true") {
        this._bhudImgCache.push(ImageManager.loadBHud("HP_Meter"))
    };
    if (String(Moghunter.bhud_mp_meter_visible) == "true") {
        this._bhudImgCache.push(ImageManager.loadBHud("MP_Meter"))
    };
    if (String(Moghunter.bhud_tp_meter_visible) == "true") {
        this._bhudImgCache.push(ImageManager.loadBHud("TP_Meter"))
    };
    if (String(Moghunter.bhud_at_meter_visible) == "true") {
        this._bhudImgCache.push(ImageManager.loadBHud("ATB_Meter"))
    };
    if (String(Moghunter.bhud_hp_number_visible) == "true") {
        this._bhudImgCache.push(ImageManager.loadBHud("HP_Number"))
    };
    if (String(Moghunter.bhud_mp_number_visible) == "true") {
        this._bhudImgCache.push(ImageManager.loadBHud("MP_Number"))
    };
    if (String(Moghunter.bhud_tp_number_visible) == "true") {
        this._bhudImgCache.push(ImageManager.loadBHud("TP_Number"))
    };
    if (String(Moghunter.bhud_maxhp_number_visible) == "true") {
        this._bhudImgCache.push(ImageManager.loadBHud("HP_Number2"))
    };
    if (String(Moghunter.bhud_maxmp_number_visible) == "true") {
        this._bhudImgCache.push(ImageManager.loadBHud("MP_Number2"))
    };
    if (String(Moghunter.bhud_maxtp_number_visible) == "true") {
        this._bhudImgCache.push(ImageManager.loadBHud("TP_Number2"))
    };
};
...
Spriteset_Map.prototype.updateToolSprite = function () {
    if ($gameSystem._eventDataToolRequestAddSprite) {
        this.addToolSprite()
    };
    if ($gameSystem._eventDataToolRequestRemoveSprite) {
        this.removeToolSprite()
    };
};
...
Sprite_Character.prototype.setCharacter = function (character) {
    ...
    if (this._character.battler()) {
        this._imgIcon = ImageManager.loadSystem("IconSet")
    };
};
...
Sprite_Character.prototype.updateSpriteCharacterCN = function () {
    ...
    if (this.needUpdateToolOffset()) {
        this.updateToolOffset()
    };
};
...
Sprite_Character.prototype.updateBattler = function () {
    if (!this._character._tool.active) {
        ...
        if ($gameSystem.isChronoMode()) {
            if (this._character._user.treasure[0]) {
                ...
                if (!Imported.MOG_CharacterMotion) {
                    this.rotation = this._character._user.rotation[0]
                };
            };
    ...
    if (this._character._user.collapse[1] > 0) {
        this.updateCollapse()
    };
    if (this._character._chrono.escaped) {
        this.faceCharacterCR(-5, true, false)
    };
};
...
Sprite_Character.prototype.faceCharacterCR = function (value, erase, collapse) {
    ...
    if (erase && this._character._opacity === 0) {
        if (collapse) {
            this._character._user.collapse[1] = 0;
        };
...
Sprite_Character.prototype.updateToolOffset = function () {
    if (this._character._user.hookshotTool) {
        this.updateHookShotLock()
    };
...
Sprite_Animation.prototype.update = function () {
    ...
    if (this._target && this._target._character) {
        this.updateChronoAnimation(this._target)
    }
};
...
Sprite_Character.prototype.updateBitmap = function () {
    if (this.needRefreshSprtPar()) {
        this.refreshCharSpritePar()
    };
...
Sprite_Character.prototype.setTileBitmap = function () {
    ...
    if (this._character) {
        this._character._spritePar.refresh = true
    };
};
...
Sprite_Character.prototype.needRefreshSprtPar = function () {
    ...
    if (!this._character._spritePar.refresh) {
        return false
    };
    if (!this.bitmap) {
        return false
...
Game_Chrono.prototype.updateChronoTouchInput = function () {
    if (this.canTouchInput()) {
        this.updateTouchInputBase()
    };
};
...
Game_Chrono.prototype.canTouchInput = function () {
    if (!$gameSystem._chronoMode.touchInput) {
        return false
    };
    if ($gameTemp._chrono.buttonLag > 0) {
        return false
