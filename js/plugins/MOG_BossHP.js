...
/*:
 * @plugindesc (v2.4)[v1.2]  敌人 - boss生命固定框（佳哥RPG修改版）
...
　　var Moghunter = Moghunter || {}; 
  ...
  　Moghunter.parameters = PluginManager.parameters('MOG_BossHP');
	...
	Moghunter.bosshp_number_max = Number(Moghunter.parameters['生命数值最大显示位'] || 8);
...
Sprite_BossHP.prototype.update_shake_effect = function() {
	...
	this._hp_data[7] -= 1;
	this.y = -3 + this._hp_data[10] + Math.floor(Math.random() * 8);
...
Sprite_BossHP.prototype.update_hp_number = function() {
	  ...
	  if (this._init && $gameSystem._battler_bhp_oldNumber[1] != 8) {
		  this._hp_data[8] = $gameSystem._battler_bhp_oldNumber[1];
		  $gameSystem._battler_bhp_oldNumber[1] = 8;
	  };	  
      var nspd = 1 + Math.floor((Math.abs(this._hp_data[8] - this._battler.hp) / 30))
      if (this._hp_data[8] < this._battler.hp) {
		  this._hp_data[6] += nspd;
		  if (this._hp_data[6] > this._battler.hp) {this._hp_data[6] = this._battler.hp};
	  }
	  else if (this._hp_data[8] > this._battler.hp) {
		  this._hp_data[8] -= nspd;
		  if (this._hp_data[8] < this._battler.hp) {this._hp_data[8] = this._battler.hp};
	  };
