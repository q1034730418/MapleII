...
// ------------------------------------------------------
/*:
 * @plugindesc (v1.41)详细信息窗口 - 支持镶嵌与江湖梦网拍卖行
 * @author Yana（Drill_up翻译+优化）Revise by Fanzi
 *
 * @param 是否初始启用
 * @type boolean
 * @on 启用
 ...
 * 添加了显示数值小数点位数。
 * [v1.4]
 * 优化了角色攻击等属性为小数时显示过长的问题。
 * [v1.41]
 * 支持YEP强化、镶嵌信息显示，支持独立物品描述。
 * 拓展江湖梦网商城、拍卖行与自动贩售支持显示。
 * ------------------------------------------------------
 * ----利用規約 （Yana）
 ...
 */
    ...
    DrillUp.useSceneShopping = DrillUp.parameters['是否应用于商城界面'] === 'true';
    DrillUp.m_shopping_offset_fix = String(DrillUp.parameters['是否固定商城界面坐标'] || "false");
    DrillUp.m_shopping_offsetX = Number(DrillUp.parameters['平移-商城界面窗口 X'] || 0);
    DrillUp.m_shopping_offsetY = Number(DrillUp.parameters['平移-商城界面窗口 Y'] || 0);
    DrillUp.useSceneStore = DrillUp.parameters['是否应用于拍卖行界面'] === 'true';
    DrillUp.m_Store_offset_fix = String(DrillUp.parameters['是否固定拍卖行界面坐标'] || "true");
    DrillUp.m_Store_offsetX = Number(DrillUp.parameters['平移-拍卖行界面窗口 X'] || 0);
    DrillUp.m_Store_offsetY = Number(DrillUp.parameters['平移-拍卖行界面窗口 Y'] || 0);
    DrillUp.useSceneAuto = DrillUp.parameters['是否应用于批量贩售'] === 'true';
    DrillUp.m_Auto_offset_fix = String(DrillUp.parameters['是否固定批量贩售坐标'] || "true");
    DrillUp.m_Auto_offsetX = Number(DrillUp.parameters['平移-批量贩售窗口 X'] || 0);
    DrillUp.m_Auto_offsetY = Number(DrillUp.parameters['平移-批量贩售窗口 Y'] || 0);
    DrillUp.useSceneMac = DrillUp.parameters['是否应用于Mac界面'] === 'true';
    DrillUp.m_Mac_offset_fix = String(DrillUp.parameters['是否固定Mac界面坐标'] || "true");
    DrillUp.m_Mac_offsetX = Number(DrillUp.parameters['平移-Mac界面窗口 X'] || 0);
    DrillUp.m_Mac_offsetY = Number(DrillUp.parameters['平移-Mac界面窗口 Y'] || 0);
    ...
    DrillUp.m_attr_float_acc = Number(DrillUp.parameters['显示数值小数点位数'] || 1);
    ...
    DataManager.makeInfoItem = function(item) {
        ...
        if (item.baseItemId) {
            if (DataManager.isWeapon(item)) texts = $dataWeapons[item.baseItemId].note.split('\n');
            else if (DataManager.isArmor(item)) texts = $dataArmors[item.baseItemId].note.split('\n');
            else texts = $dataItems[item.baseItemId].note.split('\n');
        } else if (!item.note) return;
    ...
    Window_MiniInfo.prototype.updatePosition = function(rect, padding) {
		...
		}else if( DrillUp.is_in_window === 7 ){
			if( DrillUp.m_shopping_offset_fix === "true" ){
				this.x = DrillUp.m_shopping_offsetX;
				this.y = DrillUp.m_shopping_offsetY;
			}else{
				this.x += DrillUp.m_shopping_offsetX;
				this.y += DrillUp.m_shopping_offsetY;
			}
		}else if( DrillUp.is_in_window === 8 ){
			if( DrillUp.m_Store_offset_fix === "true" ){
				this.x = DrillUp.m_Store_offsetX;
				this.y = DrillUp.m_Store_offsetY;
			}else{
				this.x += DrillUp.m_Store_offsetX;
				this.y += DrillUp.m_Store_offsetY;
			}
		}else if( DrillUp.is_in_window === 9 ){
			if( DrillUp.m_Auto_offset_fix === "true" ){
				this.x = DrillUp.m_Auto_offsetX;
				this.y = DrillUp.m_Auto_offsetY;
			}else{
				this.x += DrillUp.m_Auto_offsetX;
				this.y += DrillUp.m_Auto_offsetY;
			}
		}else if( DrillUp.is_in_window === 10 ){
			if( DrillUp.m_Mac_offset_fix === "true" ){
				this.x = DrillUp.m_Mac_offsetX;
				this.y = DrillUp.m_Mac_offsetY;
			}else{
				this.x += DrillUp.m_Mac_offsetX;
				this.y += DrillUp.m_Mac_offsetY;
			}
    ...
    Window_MiniInfo.prototype.makeContents = function() {
                ...
                switch(e.code) {
                    case 11:
						if(DrillUp.m_effect_recoverHP_visible === "true"){
							...
							if(e.value1 > 0 && e.value2 > 0){
								text += g + (e.value1*100).toFixed(DrillUp.m_attr_float_acc) + '%' + "+" + Math.floor(e.value2);
							}else if(e.value1 > 0 && e.value2 == 0 ){
								text += g + (e.value1*100).toFixed(DrillUp.m_attr_float_acc) + '%';
							}else if(e.value2 > 0 && e.value1 == 0){
								text += g + Math.floor(e.value2);
							}else{
								text += c + (e.value1*100).toFixed(DrillUp.m_attr_float_acc) + '%' + "+" + Math.floor(e.value2);
							}
						};
						if(DrillUp.m_effect_lostHP_visible === "true"){
							if(e.value1 < 0 && e.value2 < 0){
								text = s + DrillUp.m_effect_lostHP_text + ':';
								text += r + Math.abs((e.value1*100).toFixed(DrillUp.m_attr_float_acc)) + '%' + "+" + Math.abs(Math.floor(e.value2));
							}else if(e.value1 < 0 && e.value2 == 0 ){
								text = s + DrillUp.m_effect_lostHP_text + ':';
								text += r + Math.abs((e.value1*100).toFixed(DrillUp.m_attr_float_acc)) + '%';
							}else if(e.value2 < 0 && e.value1 == 0 ){
								text = s + DrillUp.m_effect_lostHP_text + ':';
								text += r + Math.abs((e.value2*100).toFixed(DrillUp.m_attr_float_acc));
							}
						};
                        break;
                    case 12:
						...
						if(DrillUp.m_effect_lostMP_visible === "true"){
							if(e.value1 < 0 && e.value2 < 0){
								...
								text = s + DrillUp.m_effect_lostMP_text + ':';
								text += r + Math.abs(Math.floor(e.value2));
							}
						}
                        break;
                    case 13:	//TP好像不能正常显示
                    ...
                    case 21:
						if( DrillUp.m_effect_addState_visible === "true" ){
							...
							if (state) {
								var name = state.name;
								if (e.value1 > 0 ) text = s + DrillUp.m_effect_addState_text + ':' + c + name + ' ' + Math.abs((e.value1*100).toFixed(DrillUp.m_attr_float_acc)) + '%';
							}
						}
                        break;
                    case 22:
						if( DrillUp.m_effect_removeState_visible === "true" ){
							...
							if (state) {
								var name = state.name;
								if (e.value1 > 0 ) text = s + DrillUp.m_effect_removeState_text + ':' + c + name + ' ' + Math.abs((e.value1*100).toFixed(DrillUp.m_attr_float_acc)) + '%';
							}
							break;
						}
                    case 31:
						if( DrillUp.m_effect_upParam_visible === "true" ){
							...
							if (e.value1 > 0 ) text = s + DrillUp.m_effect_upParam_text + ':' + c + name + ' ' + Math.floor(e.value1) + DrillUp.turnText;
						}
                        break;
                    case 32:
						if( DrillUp.m_effect_downParam_visible === "true" ){
							var name = TextManager.param(e.dataId);
							if (e.value1 > 0 ) text = s + DrillUp.m_effect_downParam_text + ':' + c + name + ' ' + Math.floor(e.value1) + DrillUp.turnText;
						}
                        break;
                    case 33:
                    ...
                    case 42:
                        if  ( DrillUp.m_effect_grow_visible === "true") {
                            var name = TextManager.param(e.dataId);
                            text = s + DrillUp.m_effect_grow_text + ':' + c + name + '+' + Math.floor(e.value1);
                        }
                        break;
                    case 43:
        ...
        if (item.params) {
            for (var i=0;i<8;i++) {			//基本能力值
                var value = item.params[i];
                if (value !== 0){
                    var ud = value > 0 ? g : r;
                    var sym = value > 0 ? '+' : '';
                    this._data.push(s + TextManager.param(i) + ud + sym + Math.round(Number(value)) );
                }
            }
        }
        if (item.traits) {
                ...
                switch (trait.code) {
                    ...
                    case 11:
                        if ( DrillUp.m_attr_element_visible === "true") {
                            ...
                            text = c + ele + s + DrillUp.m_attr_element_text + du + 'x' + (value*100).toFixed(DrillUp.m_attr_float_acc) + '%';
                        }
                        break;
                    case 112:
                    case 12:
                        if ( DrillUp.m_attr_weak_visible === "true") {
                            ...
                            text = c + param + s + DrillUp.m_attr_weak_text + du + 'x' + (value*100).toFixed(DrillUp.m_attr_float_acc) + '%';
                        }
                        break;
                    case 113:
                    case 13:
                        if ( DrillUp.m_attr_state_visible === "true") {
                            ...
                            text = c + state + s + DrillUp.m_attr_state_text + du + 'x' + (value*100).toFixed(DrillUp.m_attr_float_acc) + '%';
                        }
                        break;
                    case 114:
                    case 14:
                    ...
                    case 21:
                        if ( DrillUp.m_attr_common_visible === "true" && value !== 1.0) {
                            var param = TextManager.param(dataId);
                            text = s + param + ud + 'x' + Math.floor((value*100).toFixed(DrillUp.m_attr_float_acc)) + '%';
                        }
                        break;
                    case 122:	
                    case 22:	//追加能力值
                        if (DrillUp.m_attr_add0_visible === "true" && dataId === 0 && value !== 0 ) {
                            text = s + DrillUp.m_attr_add0_text + du + sym + (value*100).toFixed(DrillUp.m_attr_float_acc) + '%';
						}
                        if (DrillUp.m_attr_add1_visible === "true" && dataId === 1 && value !== 0 ) {
                            text = s + DrillUp.m_attr_add1_text + du + sym + (value*100).toFixed(DrillUp.m_attr_float_acc) + '%';
						}
                        if (DrillUp.m_attr_add2_visible === "true" && dataId === 2 && value !== 0 ) {
                            text = s + DrillUp.m_attr_add2_text + du + sym + (value*100).toFixed(DrillUp.m_attr_float_acc) + '%';
						}
                        if (DrillUp.m_attr_add3_visible === "true" && dataId === 3 && value !== 0 ) {
                            text = s + DrillUp.m_attr_add3_text + du + sym + (value*100).toFixed(DrillUp.m_attr_float_acc) + '%';
						}
                        if (DrillUp.m_attr_add4_visible === "true" && dataId === 4 && value !== 0 ) {
                            text = s + DrillUp.m_attr_add4_text + du + sym + (value*100).toFixed(DrillUp.m_attr_float_acc) + '%';
						}
                        if (DrillUp.m_attr_add5_visible === "true" && dataId === 5 && value !== 0 ) {
                            text = s + DrillUp.m_attr_add5_text + du + sym + (value*100).toFixed(DrillUp.m_attr_float_acc) + '%';
						}
                        if (DrillUp.m_attr_add6_visible === "true" && dataId === 6 && value !== 0 ) {
                            text = s + DrillUp.m_attr_add6_text + du + sym + (value*100).toFixed(DrillUp.m_attr_float_acc) + '%';
						}
                        if (DrillUp.m_attr_add7_visible === "true" && dataId === 7 && value !== 0 ) {
                            text = s + DrillUp.m_attr_add7_text + du + sym + (value*100).toFixed(DrillUp.m_attr_float_acc) + '%';
						}
                        if (DrillUp.m_attr_add8_visible === "true" && dataId === 8 && value !== 0 ) {
                            text = s + DrillUp.m_attr_add8_text + du + sym + (value*100).toFixed(DrillUp.m_attr_float_acc) + '%';
						}
                        if (DrillUp.m_attr_add9_visible === "true" && dataId === 9 && value !== 0 ) {
                            text = s + DrillUp.m_attr_add9_text + du + sym + (value*100).toFixed(DrillUp.m_attr_float_acc) + '%';
						}
                        break;
                    case 123:	
                    case 23:	//特殊能力值
						if(DrillUp.m_attr_ext0_visible === "true" && dataId === 0 && value !== 0 ){
                            text = s + DrillUp.m_attr_ext0_text + c + 'x' + (value*100).toFixed(DrillUp.m_attr_float_acc) + '%';
						}
						if(DrillUp.m_attr_ext1_visible === "true" && dataId === 1 && value !== 0 ){
                            text = s + DrillUp.m_attr_ext1_text + ud + 'x' + (value*100).toFixed(DrillUp.m_attr_float_acc) + '%';
						}
						if(DrillUp.m_attr_ext2_visible === "true" && dataId === 2 && value !== 0 ){
                            text = s + DrillUp.m_attr_ext2_text + ud + 'x' + (value*100).toFixed(DrillUp.m_attr_float_acc) + '%';
						}
						if(DrillUp.m_attr_ext3_visible === "true" && dataId === 3 && value !== 0 ){
                            text = s + DrillUp.m_attr_ext3_text + ud + 'x' + (value*100).toFixed(DrillUp.m_attr_float_acc) + '%';
						}
						if(DrillUp.m_attr_ext4_visible === "true" && dataId === 4 && value !== 0 ){
                            text = s + DrillUp.m_attr_ext4_text + du + 'x' + (value*100).toFixed(DrillUp.m_attr_float_acc) + '%';
						}
						if(DrillUp.m_attr_ext5_visible === "true" && dataId === 5 && value !== 0 ){
                            text = s + DrillUp.m_attr_ext5_text + ud + 'x' + (value*100).toFixed(DrillUp.m_attr_float_acc) + '%';
						}
						if(DrillUp.m_attr_ext6_visible === "true" && dataId === 6 && value !== 0 ){
                            text = s + DrillUp.m_attr_ext6_text + du + 'x' + (value*100).toFixed(DrillUp.m_attr_float_acc) + '%';
						}
						if(DrillUp.m_attr_ext7_visible === "true" && dataId === 7 && value !== 0 ){
                            text = s + DrillUp.m_attr_ext7_text + du + 'x' + (value*100).toFixed(DrillUp.m_attr_float_acc) + '%';
						}
						if(DrillUp.m_attr_ext8_visible === "true" && dataId === 8 && value !== 0 ){
                            text = s + DrillUp.m_attr_ext8_text + du + 'x' + (value*100).toFixed(DrillUp.m_attr_float_acc) + '%';
						}
						if(DrillUp.m_attr_ext9_visible === "true" && dataId === 9 && value !== 0 ){
                            text = s + DrillUp.m_attr_ext9_text + ud + 'x' + (value*100).toFixed(DrillUp.m_attr_float_acc) + '%';
						}
                        break;
                    case 31:
                    ...
                    case 32:
                        if (DrillUp.m_attr_atk1_visible === "true" && value > 0) {
                            var state = $dataStates[dataId].name;
                            text = s + DrillUp.m_attr_atk1_text + c + state + ' ' + Math.floor((value*100).toFixed(DrillUp.m_attr_float_acc)) + '%' ;
                        }
                        break;
                    case 33:
                        if (DrillUp.m_attr_atk2_visible === "true" && value !== 0){
							text = s + DrillUp.m_attr_atk2_text + ud + sym + Math.floor(value);
						}
                        break;
                    case 34:
                        if (DrillUp.m_attr_atk3_visible === "true" && value !== 0){
                            var ud = value > 0 ? g : r;
                            text = s + DrillUp.m_attr_atk3_text + ud + sym + Math.floor(value) + DrillUp.turnText;
                        }
                        break;
                    case 41:
                    ...
                    case 61:
                        if (DrillUp.m_attr_move_visible === "true"  && value > 0){
							text = s + DrillUp.m_attr_move_text + du + sym + (value*100).toFixed(DrillUp.m_attr_float_acc) + '%';
						}
                        break;
                    case 62:	//特殊标志
        ...
        if (Imported.YEP_X_ItemUpgrades && (DataManager.isWeapon(item) || DataManager.isArmor(item))) {
            var num = item.upgradeSlots + item.slotsApplied.length;
            if (item.upgradeSlots > 0 || item.slotsApplied.length > 0) {
                var text = Yanfly.Param.IUSSlotsText + item.upgradeSlots + '/' + num;
                this._data.push(text);
            }
            if (item.slotsApplied.length > 0 && num <= 5) {
                for (var i = 0; i < item.slotsApplied.length; i++) {
                    this._data.push(item.slotsApplied[i]);
                }
            }
        }
        if (Imported.YEP_X_AttachAugments && (DataManager.isWeapon(item) || DataManager.isArmor(item)))
            ItemManager.checkAugmentSlots(item);
        if (item.augmentSlotItems) {
            for (var i = 0; i < item.augmentSlotItems.length; i++) {
                this._data.push(item.augmentSlotItems[i]);
            }
        }
    ...
    Window_MiniInfo.prototype.refresh = function() {
        ...
        var aftnum = this._item._afterInfos ? this._item._afterInfos.length : 0;
        var asinum = this._item.augmentSlotItems ? this._item.augmentSlotItems.length : 0;
        for (var i=0,max=this._data.length;i<max;i++) {
            if (this._item.augmentSlotItems && i >= this._data.length - asinum - aftnum && i < this._data.length - aftnum) {
                ...
                this.drawSmallItem(this._data[i], x + 2, y - 6);
            } else {
                var x = 6 + Math.floor(i / (max / this._maxCols)) * Math.floor(this.contentsWidth() / 2);
                var y = ((i % Math.ceil(max / this._maxCols))+this.exRow()) * fs + oy;
    ...
    Window_MiniInfo.prototype.drawSmallItem = function(info, x, y) {
        if (info == 'none') {
            var index = Yanfly.Param.AugmentRemoveText.replace(/[^0-9]/ig,"");
            if (Yanfly.Param.AugmentNoneText.indexOf("]") != -1) {
                var noneText = Yanfly.Param.AugmentNoneText.split(']');
                var text = noneText[1];
            } else var text = Yanfly.Param.AugmentNoneText;
            this.drawSmallIcon(index, x, y + 8);
            this.changeTextColor(this.textColor(7));
            this.drawText(text, x + 22, y, 200);
        } else {
            ...
            if (item) {
                this.resetTextColor();
                this.drawSmallIcon(item.iconIndex, x, y + 8);
                this.drawText(item.name, x + 22, y, 200);
            }
        }
    };

    Window_MiniInfo.prototype.drawSmallIcon = function(iconIndex, x, y) {
        var bitmap = ImageManager.loadSystem('IconSet');
        var pw = Window_Base._iconWidth;
        var ph = Window_Base._iconHeight;
        var sx = iconIndex % 16 * pw;
    ...
    Scene_Base.prototype.createMiniWindow = function() {
        ...
        if (this._EqitemWindow) this._EqitemWindow._miniInfoWindow = this._miniWindow;
        if (this._skillWindow) this._skillWindow._miniInfoWindow = this._miniWindow;
        if (this._armorsWindow) this._armorsWindow._miniInfoWindow = this._miniWindow;
        if (this._weaponsWindow) this._weaponsWindow._miniInfoWindow = this._miniWindow;
        if (this._mapSelectSkillWindow) this._mapSelectSkillWindow._miniInfoWindow = this._miniWindow;
        if (this._MapSelectEquipWindow) this._MapSelectEquipWindow._miniInfoWindow = this._miniWindow;
        if (Imported.FTKR_STS && FTKR && FTKR.STS && this._stsSkillTreeWindow) this._stsSkillTreeWindow._miniInfoWindow = this._miniWindow;
    ...
    if(Imported.FTKR_STS && FTKR && FTKR.STS) {
        ...
        Scene_STS.prototype.createSkillTreeWindow = function() {
            ...
            if (DrillUp.useSceneFTKR) this.createMiniWindow();
            DrillUp.is_in_window = 6;
        };
    ...
    ////////////////////////////////////////////////////////////////////////////////////
    if (Imported.JHMW_ShopStore) {
        var __SShopOnline_create = Scene_ShopOnline.prototype.create;
        Scene_ShopOnline.prototype.create = function() {
            __SShopOnline_create.call(this);
            if (DrillUp.useSceneShopping) this.createMiniWindow();
            DrillUp.is_in_window = 7;
        };

    ////////////////////////////////////////////////////////////////////////////////////

        var __SStoreOnline_create = Scene_StoreOnline.prototype.create;
        Scene_StoreOnline.prototype.create = function() {
            __SStoreOnline_create.call(this);
            if (DrillUp.useSceneStore) this.createMiniWindow();
            DrillUp.is_in_window = 8;
        };
    }
    ////////////////////////////////////////////////////////////////////////////////////
    if (window['EquipAutoShop']) {
        var __SAutoShop_create = Scene_AutoShop.prototype.create;
        Scene_AutoShop.prototype.create = function() {
            __SAutoShop_create.call(this);
            if (DrillUp.useSceneAuto) this.createMiniWindow();
            DrillUp.is_in_window = 9;
        };
    }
    ////////////////////////////////////////////////////////////////////////////////////
    if (window['Mac_Menu']) {
        var __Mac_Eqmenu_create = Mac_Eqmenu.prototype.create;
        Mac_Eqmenu.prototype.create = function() {
            __Mac_Eqmenu_create.call(this);
            if (DrillUp.useSceneMac) this.createMiniWindow();
            DrillUp.is_in_window = 10;
        };
        Window_MiniInfo.prototype.drawItemName = function(item, x, y, width) {
            width = width || 312;
            if (item) {
                var iconBoxWidth = Window_Base._iconWidth + 4;
                if (DataManager.isWeapon(item) || DataManager.isArmor(item)) this.ItemColor(item);
                this.drawIcon(item.iconIndex, x + 2, y + 2);
                this.drawText(item.name, x + iconBoxWidth, y, width - iconBoxWidth);
                this.resetTextColor();
            }
        };
        Window_MiniInfo.prototype.ItemColor = function(item) {
