...
Galv.PMOVE.Game_Map_deltaX = Game_Map.prototype.deltaX;
Game_Map.prototype.deltaX = function(x1, x2) {
	x1 = Math.round(x1);
	x2 = Math.round(x2);
	return Galv.PMOVE.Game_Map_deltaX.call(this, x1, x2);
};
	
Galv.PMOVE.Game_Map_deltaY = Game_Map.prototype.deltaY;
Game_Map.prototype.deltaY = function(y1, y2) {
	y1 = Math.round(y1);
	y2 = Math.round(y2);
	return Galv.PMOVE.Game_Map_deltaY.call(this, y1, y2);
};
    ...
    if (!$gameMap.isEventRunning()) {
        $gameMap.eventsXy(x, y).forEach(function(event) {
            if (event.isTriggerIn(triggers) && event.isNormalPriority() === normal) {
				if (event.event().meta && !event.event().meta.noDelay && !$gamePlayer.tileDelayed()) $gamePlayer.setTileDelay();
                event.start();
            }
        });
    }
...
Game_Player.prototype.canPass = function(x, y, d, qx, qy) {
	...
	if (this._normMove || qx === undefined || qy === undefined){return canPassTile;}
	else {
